...
import { Crown, Award, Star, Users, Calendar, Trophy, Code, Camera, ExternalLink, Linkedin, Github, ChevronDown, ChevronUp } from 'lucide-react';
...
export function Leaders() {
  ...
  const totalTeamMembers = teams.reduce((sum, team) => sum + team.members.length, 0);
  const technicalTeamSize = teams.find(t => t.type === 'technical')?.members.length || 0;
  const mediaTeamSize = teams.find(t => t.type === 'media')?.members.length || 0;
  ...
  const getRoleColor = (role: string) => {
    if (role.includes('Advisor') || role.includes('Faculty')) return 'from-yellow-600 to-orange-600';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white section-padding">
      <div className="max-w-7xl mx-auto container-padding">
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-responsive-3xl font-bold mb-4 sm:mb-6">Our Team</h1>
          <p className="text-responsive-base text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
            Meet the {totalLeaders} dedicated leaders and {totalTeamMembers} team members driving our competitive programming excellence
          </p>
        </div>
        ...
        <section className="mb-12 sm:mb-16">
          <div className="flex flex-col sm:flex-row sm:items-center mb-6 sm:mb-8">
            <div className="flex items-center mb-2 sm:mb-0">
              ...
              <h2 className="text-responsive-xl font-bold">Current Leadership Team</h2>
            </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start space-y-4 sm:space-y-0 sm:space-x-4">
                    <div className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br ${getRoleColor(leader.role)} rounded-full flex items-center justify-center mx-auto sm:mx-0`}>
                      <Icon className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                    </div>
                    <div className="flex-1 text-center sm:text-left">
                      <div className="mb-3 sm:mb-2">
                        <div className="mb-2">
                          <h3 className="text-responsive-lg font-bold">{leader.name}</h3>
                          <p className="text-blue-600 dark:text-blue-400 font-semibold text-responsive-sm">{leader.role}</p>
                        </div>
                      </div>
                      ...
                      {leader.contact && (
                        <div className="flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm justify-center sm:justify-start">
                          {leader.contact.email && (
                            <span className="text-gray-600 dark:text-gray-400 break-all">{leader.contact.email}</span>
                          )}
                          {leader.contact.github && (
                            <span className="text-gray-600 dark:text-gray-400 break-all">@{leader.contact.github}</span>
                          )}
                          {leader.contact.cf && (
                            <a
            ...
            {teams.map((team, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                    <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                      <div className={`inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br ${
                        team.type === 'technical' ? 'from-blue-600 to-purple-600' : 'from-pink-600 to-rose-600'
                      ...
                      <div className="text-center sm:text-left">
                        <h3 className="text-responsive-xl font-bold">{team.name}</h3>
                        <p className="text-responsive-sm text-gray-600 dark:text-gray-300">{team.description}</p>
                ...
                <div className={`transition-all duration-500 ease-in-out overflow-hidden ${
                  expandedTeam === team.name ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
                }`}>
                          ...
                          {team.members.map((member, memberIndex) => (
                            ...
                            >
                              <div className="flex flex-col items-center text-center w-full flex-1">
                                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-600 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm sm:text-base mb-3 flex-shrink-0 transition-transform duration-300 hover:scale-110">
                                  {member.name.charAt(0)}
                                </div>
                                <h4 className="font-bold text-sm sm:text-base mb-1 text-center break-words w-full leading-tight">{member.name}</h4>
                                {member.handle && (
                                  <a
                                    href={`https://codeforces.com/profile/${member.handle}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 mb-2 text-center break-all transition-colors duration-200 hover:underline"
                                  >
                                    @{member.handle}
                                  </a>
                                )}
                                {member.role && (
                                  <span className="inline-block px-2 py-1 sm:px-3 bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 text-xs font-medium rounded-full mb-3 text-center max-w-full leading-tight">
                                    {member.role}
                                  </span>
                                )}
                                {member.maxRating && (
                                  <div className="mb-3">
                                    <div className={`font-bold text-sm sm:text-base ${getRatingColor(member.maxRating)}`}>
                                      {member.maxRating}
                                    </div>
                          ...
                          {team.members.map((member, memberIndex) => (
                            ...
                            >
                              <div className="flex flex-col items-center text-center w-full flex-1">
                                ...
                                <h4 className="font-bold text-sm sm:text-base mb-2 text-center break-words w-full leading-tight">{member.name}</h4>
                                {member.role && (
