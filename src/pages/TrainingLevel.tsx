...
export function TrainingLevel({ season, level, onBack }: TrainingLevelProps) {
  ...
  const getLevelDescription = (level: string) => {
    const descriptions = {
      "level 0":
        "Introduction to competitive programming fundamentals and basic problem-solving techniques",
      "level 1": "Basic algorithms, data structures, and implementation skills",
      "level 2": "Advanced algorithms, dynamic programming, and graph theory",
    };
    return (
      descriptions[level as keyof typeof descriptions] ||
      "Training content for this level"
    );
  };
  ...
  const getWeeksData = (level: string): Week[] => {
    ...
    const currentWave =
      currentSeason.waves.find((w) => w.wave === currentSeason.currentWave) ||
  ...
  const currentWeek = levelData?.currentWeek || 0;
  const currentLevel = currentWave?.levels.find((l) => l.level === level);
